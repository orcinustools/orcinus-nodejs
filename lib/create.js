...
module.exports = {
  ...
  execution: (keyMap) => {
    ...
    var cmd = "docker service create -d " + opt + " --name " + name + " " + img + " " + exec;
    //console.log(cmd);
    chp.exec(cmd, (e, stdout, stderr) => {
      if (stdout) {
  ...
  opt: (app) => {
    ...
    if (cmd.indexOf("mode") >= 0) {
      if (app.mode) {
        arr.push("--mode " + app.mode);
      }
    }
    if (cmd.indexOf("restart_policy") >= 0) {
      if (app.restart_policy.condition) {
        arr.push("--restart-condition " + app.restart_policy.condition);
      }
      if (app.restart_policy.delay) {
        arr.push("--restart-delay " + app.restart_policy.delay);
      }
    }
    ...
    if (cmd.indexOf("sysctls") >= 0) {
      app.sysctls.forEach((v) => {
        arr.push("--sysctl " + v);
      });
    }
    if (cmd.indexOf("deploy") >= 0) {
      if(app.deploy.delay) arr.push("--update-delay " + app.deploy.delay);
      if(app.deploy.iffailure) arr.push("--update-failure-action " + app.deploy.iffailure);
      if(app.deploy.order) arr.push("--update-order " + app.deploy.order);
    }
    if (cmd.indexOf("user") >= 0) {
      arr.push("--user " + app.user);
    }
