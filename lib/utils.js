  ...
  serviceManifest : (userID,req,res,callback) => {
    ...
    Object.keys(manifest.opt.services).forEach(function(v){
      ...
      if(manifest.opt.services[v].endpoint){
        ...
        // setup custom domain
        var domain = serviceName+"."+process.env.ORCINUS_DOMAIN;
        if(manifest.opt.domain){
          manifest.opt.services[v].labels["traefik.frontend.rule"] = manifest.domain;
          domain = manifest.opt.services[v].labels["traefik.frontend.rule"];
        }
        module.exports.debug(domain);
        manifest.opt.services[v].labels["orcinus.domain"] = domain;
      }
