    ...
    Object.keys(manifest.opt.services).forEach(function(v){
      ...
      if(manifest.opt.services[v].endpoint){
        ...
        if(manifest.opt.domain){
          manifest.opt.services[v].labels["traefik.frontend.rule"] = manifest.opt.domain;
          domainSVC = manifest.opt.services[v].labels["traefik.frontend.rule"];
        }
      ...
      //module.exports.debug(JSON.stringify(modifySVC));
